  ...
  static async fetchUserStats() {
    ...
    try {
      ...
      if (result.result && result.result.code === 0) {
        ...
        uni.setStorageSync(STORAGE_KEYS.USER_STATS, formattedStats)
        
        if (userActions && userActions.updateUserStats) {
          userActions.updateUserStats(formattedStats)
        }
        
        console.log('获取用户统计成功:', formattedStats)
        return formattedStats
      } else {
        const errorMsg = result.result?.message || '获取用户统计失败'
        console.error('获取用户统计失败:', errorMsg)
        throw new Error(errorMsg)
      }
    } catch (error) {
      ...
      // 如果获取失败，尝试从本地缓存获取
      const cachedStats = uni.getStorageSync(STORAGE_KEYS.USER_STATS)
      if (cachedStats) {
        console.log('使用本地缓存的统计:', cachedStats)
        return cachedStats
      }
