        ...
        success: async (res) => {
          if (res.confirm) {
            ...
            try {
              ...
              common_vendor.index.__f__("error", "at pages/game/list.vue:756", "加入游戏失败:", error);
              common_vendor.index.showToast({
        ...
        success: async (res) => {
          if (res.confirm) {
            ...
            try {
              ...
              common_vendor.index.__f__("error", "at pages/game/list.vue:824", "退出游戏失败:", error);
              common_vendor.index.showToast({
    ...
    const editGame = (game) => {
      common_vendor.index.navigateTo({
        ...
        events: {
          // 监听编辑页面的事件
          refreshList: async () => {
            common_vendor.index.__f__("log", "at pages/game/list.vue:865", "收到编辑页面刷新事件");
            common_vendor.index.setStorageSync("needRefreshGameList", true);
            await loadGameList();
            updateTabCounts();
          }
        ...
        success: (res) => {
          res.eventChannel.on("onGameUpdated", async (data) => {
            common_vendor.index.__f__("log", "at pages/game/list.vue:876", "编辑页面关闭，收到组局更新事件:", data);
            if (data.success) {
              common_vendor.index.setStorageSync("needRefreshGameList", true);
              await loadGameList();
              updateTabCounts();
          ...
          if (res.confirm) {
            ...
            try {
              ...
              if (result && result.errMsg === "cloud.callFunction:ok" && result.result.code === 0) {
                ...
                common_vendor.index.__f__("log", "at pages/game/list.vue:950", "组局取消成功，已重新加载列表");
              } else {
                common_vendor.index.__f__("error", "at pages/game/list.vue:953", "取消失败返回结果:", result);
                throw new Error(((_a = result == null ? void 0 : result.result) == null ? void 0 : _a.message) || "取消失败");
              }
            } catch (error) {
              common_vendor.index.__f__("error", "at pages/game/list.vue:958", "取消组局失败:", error);
              common_vendor.index.showToast({
