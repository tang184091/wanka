...
class UserService {
  ...
  static async cloudLogin(code, userInfo) {
    ...
    try {
      ...
      common_vendor.index.__f__("error", "at utils/user.js:102", "云函数返回错误:", errorMsg);
  ...
  static async promptCaptcha(captchaHint = "请输入验证码") {
    return new Promise((resolve, reject) => {
      common_vendor.index.showModal({
        ...
        fail: (err) => {
          reject(new Error((err == null ? void 0 : err.errMsg) || "验证码输入失败"));
        }
      });
    });
  }
  ...
  static saveUserData(userInfo, token, stats = null) {
    ...
    common_vendor.index.__f__("log", "at utils/user.js:172", "保存用户数据:", { userInfo, token, stats });
  ...
  static logout() {
    common_vendor.index.__f__("log", "at utils/user.js:204", "开始执行 UserService.logout() - 只处理数据清理");
    const storageKeys = [
    ...
    storageKeys.forEach((key) => {
      common_vendor.index.__f__("log", "at utils/user.js:215", `清除存储: ${key}`);
      common_vendor.index.removeStorageSync(key);
    });
    common_vendor.index.__f__("log", "at utils/user.js:219", "✅ 本地存储已清除");
    common_vendor.index.__f__("log", "at utils/user.js:226", "触发 user:logout 事件，通知所有监听组件");
    common_vendor.index.$emit("user:logout");
    common_vendor.index.__f__("log", "at utils/user.js:229", "✅ UserService.logout() 执行完成");
    return true;
  }
  // 更新用户信息
  static async updateUserInfo(updates) {
    ...
    try {
      ...
      common_vendor.index.__f__("log", "at utils/user.js:253", "更新用户信息结果:", result);
      if (result.result && result.result.code === 0) {
      ...
      common_vendor.index.__f__("error", "at utils/user.js:274", "更新用户信息失败:", error);
      throw error;
    }
  }
  // 更新用户标签
  static async updateUserTags(tags) {
    ...
    try {
      ...
      common_vendor.index.__f__("error", "at utils/user.js:313", "更新用户标签失败:", error);
      throw error;
    }
  }
  // 更新用户标签
  static async updateUserTags(tags) {
    ...
    try {
      ...
      common_vendor.index.__f__("error", "at utils/user.js:352", "更新用户标签失败:", error);
      throw error;
    }
  }
  // 更新用户游戏/装备
  static async updateUserGames(games) {
    ...
    try {
      ...
      common_vendor.index.__f__("error", "at utils/user.js:391", "更新用户游戏失败:", error);
      throw error;
    }
  }
  // 更新用户头像
  static async updateUserAvatar(avatarUrl, avatarSize = 0) {
    ...
    try {
      ...
      common_vendor.index.__f__("error", "at utils/user.js:431", "更新用户头像失败:", error);
      throw error;
    }
  }
  // 获取用户统计
  static async fetchUserStats() {
    ...
    try {
      ...
      if (result.result && result.result.code === 0) {
        ...
        common_vendor.index.__f__("log", "at utils/user.js:470", "获取用户统计成功:", formattedStats);
        return formattedStats;
      } else {
        const errorMsg = ((_a = result.result) == null ? void 0 : _a.message) || "获取用户统计失败";
        common_vendor.index.__f__("error", "at utils/user.js:474", "获取用户统计失败:", errorMsg);
        throw new Error(errorMsg);
      }
    } catch (error) {
      common_vendor.index.__f__("error", "at utils/user.js:478", "获取用户统计失败:", error);
      const cachedStats = common_vendor.index.getStorageSync(STORAGE_KEYS.USER_STATS);
      if (cachedStats) {
        common_vendor.index.__f__("log", "at utils/user.js:483", "使用本地缓存的统计:", cachedStats);
        return cachedStats;
      }
      return {
        createdGames: 0,
  ...
  static async uploadImage(filePath) {
    ...
    try {
      ...
      common_vendor.index.__f__("log", "at utils/user.js:519", "开始上传图片到云存储:", cloudPath);
      const uploadResult = await common_vendor.wx$1.cloud.uploadFile({
        cloudPath,
        filePath
      });
      common_vendor.index.__f__("log", "at utils/user.js:526", "图片上传成功:", uploadResult.fileID);
      return uploadResult.fileID;
    } catch (error) {
      common_vendor.index.__f__("error", "at utils/user.js:529", "图片上传失败:", error);
      throw error;
    }
  }
  // 清理所有本地数据（调试用）
  static clearAllData() {
    common_vendor.index.__f__("log", "at utils/user.js:536", "清理所有本地数据");
    const keys = [
      STORAGE_KEYS.USER_INFO,
      STORAGE_KEYS.USER_TOKEN,
      STORAGE_KEYS.USER_STATS,
