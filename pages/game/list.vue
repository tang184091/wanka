                ...
                <view class="game-footer">
                  <view class="players-info">
                    <view class="avatars">
                      ...
                      >
                        <image :src="player.avatar" class="player-avatar" />
                      </view>
                      <view v-if="game.currentPlayers > 4" class="more-players">
                        +{{ game.currentPlayers - 4 }}
                      </view>
                    </view>
                    <text class="players-count">{{ game.currentPlayers || 1 }}/{{ game.maxPlayers }}人</text>
                  </view>
...
const loadGameList = async () => {
  ...
  try {
    ...
    if (activeTab.value === 'created') {
      console.log('开始获取我创建的组局...')
      console.log('当前用户ID:', userId.value)
      
      try {
        const result = await gameActions.getMyGames('created')
        console.log('getMyGames 返回结果:', result)
        
        if (result && result.code === 0) {
          // 处理返回的数组数据
          const gamesData = result.data || []
          console.log('获取到的游戏数据:', gamesData)
          ...
          console.log('处理后games.value:', games.value)
        } else {
          console.error('返回结果格式错误:', result)
...
const updateTabCounts = () => {
  tabs.value.forEach(tab => {
    ...
    switch (tab.id) {
      case 'created':
        count = games.value.filter(g => g.creatorId === userId.value).length
        break
      case 'joined':
        count = games.value.filter(g => g.participants && g.participants.includes(userId.value)).length
        break
      case 'pending':
  ...
  uni.showModal({
    ...
    success: async (res) => {
      if (res.confirm) {
        ...
        try {
          const result = await gameActions.joinGame(game.id || game._id)
  ...
  uni.showModal({
    ...
    success: async (res) => {
      if (res.confirm) {
        ...
        try {
          const result = await gameActions.quitGame(game.id || game._id)
  ...
  uni.showModal({
    ...
    success: async (res) => {
      if (res.confirm) {
        ...
        try {
          const result = await gameActions.deleteGame(game.id || game._id)
