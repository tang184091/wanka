...
diff --git a/pages/record/record.vue b/pages/record/record.vue
new file mode 100644
index 0000000000000000000000000000000000000000..cf71e64910921c474fc2f374afc73131f8d5c6a4
--- /dev/null
+++ b/pages/record/record.vue
@@ -0,0 +1,171 @@
+<template>
...
+            <text class="name">{{ player.nickname || player.userId }}</text>
...
+        <view class="hint">支持输入 4 名玩家 ID，分数总和必须为 100000（或 1000）</view>
+
...
+
+      <view class="card">
+        <view class="card-title">导出文本</view>
+        <view class="export-box">{{ exportText }}</view>
+        <view class="btn secondary" @tap="copyExport">复制</view>
+      </view>
+    </scroll-view>
+  </view>
+</template>
...
+  const ids = form.value.players.map(p => p.userId || '-').join(' ')
+  const scores = form.value.players.map(p => p.score || '0').join(' ')
+  return `${ids}\n${scores}`
+})
+
+const formatTime = (t) => {
...
+.export-box { white-space: pre-line; margin-top: 12rpx; background: #f8fafc; border-radius: 10rpx; padding: 14rpx; }
