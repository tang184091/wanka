<template>
  <view class="create-container">
    <scroll-view class="form-scroll" scroll-y>
      ...
      <!-- 预览 -->
      <view v-if="formData.title || formData.project" class="form-section">
        <view class="section-title">活动预览</view>
        <view class="preview-card card">
          <view class="card-header">
            <view class="game-type">
              <view :class="['type-tag', getTypeClass(formData.type)]">
                {{ getTypeText(formData.type) }}
              </view>
            </view>
            <view class="status-tag tag-status">
              {{ formData.maxPlayers }}人局
            </view>
          </view>

          <view v-if="formData.title" class="game-title">
            <text class="title-text">{{ formData.title }}</text>
          </view>
          <view v-if="formData.project" class="game-project">
            <text class="project-text">{{ formData.project }}</text>
          </view>

          <view class="preview-info">
            <view v-if="formData.time" class="info-item">
              <image src="/static/icons/time.png" class="info-icon" />
              <text class="info-text">{{ formatDateTime(formData.time) }}</text>
            </view>
            <view v-if="formData.location" class="info-item">
              <image src="/static/icons/location.png" class="info-icon" />
              <text class="info-text">{{ formData.location }}</text>
            </view>
            <view class="info-item">
              <image src="/static/icons/people.png" class="info-icon" />
              <text class="info-text">0/{{ formData.maxPlayers }}人（待加入）</text>
            </view>
          </view>
          ...
          <view class="preview-players">
            ...
            <text class="player-tips">等待玩家加入...</text>
          </view>
        </view>
      </view>
...
// 响应式数据
const gameTypes = ref([
  { id: 'mahjong', name: '日麻', icon: '/static/icons/mahjong.png' },
  { id: 'boardgame', name: '桌游', icon: '/static/icons/boardgame.png' },
  { id: 'videogame', name: '电玩', icon: '/static/icons/videogame.png' }
])
...
const playerConfig = {
  mahjong: { min: 3, max: 4 },
  boardgame: { min: 2, max: 10 },
  videogame: { min: 2, max: 8 }
}

// 计算属性
const minPlayers = computed(() => playerConfig[formData.value.type]?.min || 2)
const maxPlayers = computed(() => playerConfig[formData.value.type]?.max || 4)
...
const selectType = (type) => {
  ...
  if (config) {
    formData.value.maxPlayers = config.min
    
    // 如果是桌游，默认设置为4人（比较常见）
    if (type === 'boardgame') {
      formData.value.maxPlayers = 4
    }
  }
...
const getPlayerRangeText = () => {
  const config = playerConfig[formData.value.type]
  if (!config) return '2-4人'
  
  if (config.min === config.max) {
    return `仅限${config.min}人`
  } else {
    return `${config.min}-${config.max}人`
  }
}
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const result = await wx.cloud.callFunction({
      ...
      data: {
        action: 'createGame',
        data: {  // 注意：这里必须有 data 字段
          gameData: gameData  // 这里必须有 gameData 字段
        }
      }
    })
    ...
    // 更友好的错误提示
    let errorMessage = error.message || '创建失败，请重试'
